...
public final class Detector
{
  ...
  private int getColor(Point paramPoint1, Point paramPoint2)
  {
    ...
    for (int j = 0; j < f1; j++)
    {
      f4 += f2;
      f5 += f3;
      if (this.image.get(MathUtils.round(f4), MathUtils.round(f5)) != bool) {
        i++;
      }
    }
    float f6 = i / f1;
  ...
  private Point getMatrixCenter()
  {
    ...
    catch (NotFoundException localNotFoundException1)
    {
      try
      {
        ...
        return new Point(MathUtils.round((localResultPoint5.getX() + localResultPoint8.getX() + localResultPoint6.getX() + localResultPoint7.getX()) / 4.0F), MathUtils.round((localResultPoint5.getY() + localResultPoint8.getY() + localResultPoint6.getY() + localResultPoint7.getY()) / 4.0F), null);
